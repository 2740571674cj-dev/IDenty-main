...
const OAUTH_CLIENT_ID = (process.env.OAUTH_CLIENT_ID || DEFAULT_OAUTH_CLIENT_ID).trim() || DEFAULT_OAUTH_CLIENT_ID;
const OAUTH_PROMPT = (process.env.OAUTH_PROMPT || '').trim();
const OAUTH_ORIGINATOR = (process.env.OAUTH_ORIGINATOR || 'codex_cli_rs').trim() || 'codex_cli_rs';
const OAUTH_SIMPLIFIED_FLOW = !/^(0|false|no)$/i.test(String(process.env.OAUTH_SIMPLIFIED_FLOW || 'true').trim());
const OAUTH_ID_TOKEN_ADD_ORGS = !/^(0|false|no)$/i.test(String(process.env.OAUTH_ID_TOKEN_ADD_ORGS || 'true').trim());
const OAUTH_ALLOWED_WORKSPACE_ID = (process.env.OAUTH_ALLOWED_WORKSPACE_ID || '').trim();
 ...
 */
export function getAuthorizeUrl(redirectUri, options = {}) {
  pruneState();
  const { code_verifier, code_challenge } = generatePKCE();
  const state = generateState();
  stateStore.set(state, { code_verifier, createdAt: Date.now() });
  const legacyMode = options && options.legacy === true;
  ...
  if (OAUTH_PROMPT) params.set('prompt', OAUTH_PROMPT);
  if (!legacyMode) {
    if (OAUTH_ID_TOKEN_ADD_ORGS) params.set('id_token_add_organizations', 'true');
    if (OAUTH_SIMPLIFIED_FLOW) params.set('codex_cli_simplified_flow', 'true');
    if (OAUTH_ORIGINATOR) params.set('originator', OAUTH_ORIGINATOR);
    if (OAUTH_ALLOWED_WORKSPACE_ID) params.set('allowed_workspace_id', OAUTH_ALLOWED_WORKSPACE_ID);
  }
...
export function getOAuthRuntimeConfig() {
  return {
    OAUTH_CLIENT_ID,
    OAUTH_PROMPT: OAUTH_PROMPT || null,
    OAUTH_ORIGINATOR,
    OAUTH_SIMPLIFIED_FLOW,
    OAUTH_ID_TOKEN_ADD_ORGS,
    OAUTH_ALLOWED_WORKSPACE_ID: OAUTH_ALLOWED_WORKSPACE_ID || null,
  };
}
